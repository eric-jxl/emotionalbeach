// Package db provides Wire-compatible providers for database connections.
package db
...
// Set is the Wire provider set for all database dependencies.
var Set = wire.NewSet(ProvideDB, ProvideRedis)
...
// The returned cleanup func closes every connection pool.
...
// Returns a nil client (no error) when Redis is disabled so downstream
// components can optional-check for nil.
